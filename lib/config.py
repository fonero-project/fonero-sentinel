...
sentinel_cfg = FoneroConfig.tokenize(sentinel_config_file)
sentinel_version = "1.2.1"
min_fonerod_proto_version_with_sentinel_ping = 70207


def get_fonero_conf():
    if sys.platform == 'win32':
        fonero_conf = os.path.join(os.getenv('APPDATA'), "FoneroCore/fonero.conf")
    else:
        home = os.environ.get('HOME')

        fonero_conf = os.path.join(home, ".fonerocore/fonero.conf")
        if sys.platform == 'darwin':
            fonero_conf = os.path.join(home, "Library/Application Support/FoneroCore/fonero.conf")

    fonero_conf = sentinel_cfg.get('fonero_conf', fonero_conf)
