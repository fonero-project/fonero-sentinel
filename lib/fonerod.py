...
class FoneroDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some fonerod methods
    ...
    @classmethod
    def from_fonero_conf(self, fonero_dot_conf):
        from fonero_config import FoneroConfig
        config_text = FoneroConfig.slurp_config_file(fonero_dot_conf)
        creds = FoneroConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = fonerolib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = fonerolib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = fonerolib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_fonerod_proto_version_with_sentinel_ping)
