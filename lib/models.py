...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, fonerod):
        golist = fonerod.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_fonerod(fonerod, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_fonerod(self, fonerod, rec):
        import decimal
        import fonerolib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from fonerod...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from fonerod, with every run
        ...
        # get/create, then sync payment amounts, etc. from fonerod - Fonerod is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(fonerod)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from fonerod! %s" % e)
            govobj.vote_delete(fonerod)
            return (govobj, None)
    ...
    def vote_delete(self, fonerod):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(fonerod, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, fonerod, signal, outcome):
        import fonerolib
        ...
        output = fonerod.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = fonerolib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(fonerod, signal)

    def sync_network_vote(self, fonerod, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = fonerod.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = FNOD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 fonero addr, non-multisig
            if not fonerolib.is_valid_fonero_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Fonero address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Fonero Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (fonerolib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = FNOD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not fonerolib.is_valid_fonero_address(addr, config.network):
    ...
    def hash(self):
        import fonerolib
        return fonerolib.hashit(self.serialise())
