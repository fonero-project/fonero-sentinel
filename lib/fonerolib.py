...
def is_valid_fonero_address(address, network='mainnet'):
    ...
    fonero_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Fonero address
        return False

    if (address_version != fonero_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.fonerod_serialise())
...
def SHIM_serialise_for_fonerod(sentinel_hex):
    ...
    # superblock => "trigger" in fonerod
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # fonerod expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    fonerod_hex = serialise(obj)
    return fonerod_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('fonero.conf')
