...
class GovernanceClass(object):
    ...
    def vote(self, fonerod, signal, outcome):
        return self.go.vote(fonerod, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, fonerod):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(fonerod, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(fonerod, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import fonerolib
        obj_data = fonerolib.SHIM_serialise_for_fonerod(self.serialise())
    ...
    def fonerod_serialise(self):
        import fonerolib
        return fonerolib.SHIM_serialise_for_fonerod(self.serialise())
