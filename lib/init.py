...
def has_fonero_conf():
    ...
    valid_fonero_conf = False

    # ensure fonero_conf exists & readable
    #
    # if not, print a message stating that Fonero Core must be installed and
    # configured, including JSONRPC access in fonero.conf
    try:
        f = io.open(config.fonero_conf)
        valid_fonero_conf = True
    except IOError as e:
        print(e)

    return valid_fonero_conf
...
def main():
    ...
    if not has_fonero_conf():
        print("FoneroCore must be installed and configured, including JSONRPC access in fonero.conf")
        sys.exit(1)
